...
function renderBanner(state) {
  ...
  const statusLabel = { in_progress:'Generando…', paused_quota:'⏸ Pausado: límite de cuota (429) de la API alcanzado', paused_network:'⏸ Pausado: error de red', paused_error:'⏸ Pausado: la respuesta no fue válida', paused_manual:'⏸ Pausado manualmente' }[state.status] || state.status;
  const isError = state.status.startsWith('paused');
  slot.innerHTML = `<div class="banner ${isError ? 'error' : ''}"><div style="flex:1; min-width:200px;"><b>${escapeHtml(state.chapterTitle)}</b> · ${statusLabel}<div class="progress-track"><div class="progress-fill" style="width:${pct}%"></div></div><div class="muted">${state.wordsSoFar} / ${state.targetWords} palabras · bloque ${state.blocksDone}</div>${state.lastError ? `<div class="muted">${escapeHtml(state.lastError.message)}</div>` : ''}</div><div class="btn-row" style="margin-top:0;">${state.status === 'in_progress' ? `<button class="btn btn-ghost btn-sm" id="gen-stop-btn">Detener</button>` : `<button class="btn btn-primary btn-sm" id="gen-resume-btn">Reanudar</button><button class="btn btn-danger btn-sm" id="gen-discard-btn">Descartar</button>`}</div></div>`;
