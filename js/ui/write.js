...
function showPaper(state) { const card = document.getElementById('w-paper-card'); if (!card) return; card.style.display = 'block'; document.getElementById('w-paper-title').textContent = state.chapterTitle; document.getElementById('w-paper-meta').textContent = `${state.wordsSoFar} / ${state.targetWords} palabras`; document.getElementById('w-paper-text').innerHTML = renderManuscript(state.accumulatedText || '(aún sin texto)'); }
async function onGenerateClick() {
  const title = document.getElementById('w-title').value.trim(); const instructions = document.getElementById('w-instructions').value.trim(); const targetWords = parseInt(document.getElementById('w-words').value, 10); const requiredEnding = document.getElementById('w-ending')?.value.trim() || '';
  ...
  await runGeneration({ chapterId: chapter.id, chapterTitle: title, instructions, targetWords, requiredEnding });
}
async function resumeGeneration(state) { await runGeneration({ chapterId: state.chapterId, chapterTitle: state.chapterTitle, instructions: state.instructions, targetWords: state.targetWords, requiredEnding: state.requiredEnding || '' }); }
async function runGeneration({ chapterId, chapterTitle, instructions, targetWords, requiredEnding = '' }) {
  ...
  const finalState = await startOrResumeGeneration({ chapterId, chapterTitle, instructions, targetWords, requiredEnding, shouldStop: () => stopRequested, onProgress: ({ phase, state }) => { renderBanner(state); showPaper(state); } });
